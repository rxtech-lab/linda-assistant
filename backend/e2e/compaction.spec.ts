...
async function sendAndWait(request: APIRequestContext, sessionId: string, content: string) {
  const res = await request.post(`/api/chat-sessions/${sessionId}/messages`, { data: { content } });
...
test.describe("Compaction", () => {
  ...
  test("compaction triggers and emits compacting event after many messages", async ({
    ...
    const msgsBefore = await request.get(`/api/chat-sessions/${sessionId}/messages`);
  ...
  test("session detail endpoint returns all messages including compacted", async ({
    ...
    const msgsBeforeRes = await request.get(`/api/chat-sessions/${sessionId}/messages`);
    ...
    expect(detailBody.messages.length).toBeGreaterThanOrEqual(messageCountBefore + 2);
  ...
  test("agent works correctly after compaction on subsequent messages", async ({
    ...
    const followUpPromise = consumeSSE(`${baseURL}/api/chat-sessions/${sessionId}/stream`, {
      headers: { authorization: "Bearer e2e-test-token" },
      timeoutMs: 15000,
    });
    await new Promise((r) => setTimeout(r, SUB_DELAY));

    const followUpRes = await request.post(`/api/chat-sessions/${sessionId}/messages`, {
  ...
  test("agent responds without error to oversized message", async ({ request, baseURL }) => {
    // Create session
    const sessionRes = await request.post("/api/chat-sessions", {
      data: { assigneeId },
    });
  ...
  test("paginated messages endpoint returns compacted messages", async ({ request, baseURL }) => {
    // Create session
    const sessionRes = await request.post("/api/chat-sessions", {
      data: { assigneeId },
    });
    ...
    const activeMsgs = msgsBody.messages.filter((m: { isCompacted?: boolean }) => !m.isCompacted);
    expect(compactedMsgs.length).toBeGreaterThan(0);
    expect(activeMsgs.length).toBeGreaterThan(0);
    ...
    const page1Res = await request.get(`/api/chat-sessions/${sessionId}/messages?limit=5`);
