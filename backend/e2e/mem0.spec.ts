...
test.describe("Mem0 API", () => {
  ...
  test("GET /memories → list memories", async ({ request }) => {
    const res = await request.get(`${MEM0_URL}/memories?user_id=${TEST_USER_ID}`);
    await expectOk(res);
    const body = await res.json();
    expect(body.results).toBeDefined();
    expect(Array.isArray(body.results)).toBe(true);
  ...
  test("GET /memories/:id → get single memory", async ({ request }) => {
    const listRes = await request.get(`${MEM0_URL}/memories?user_id=${TEST_USER_ID}`);
  ...
  test("DELETE /memories/:id → delete single memory", async ({ request }) => {
    const addRes = await request.post(`${MEM0_URL}/memories`, {
      data: {
        messages: [{ role: "user", content: "I like pineapple on pizza." }],
        user_id: TEST_USER_ID,
      },
    });
    ...
    if (memoryId) {
      const res = await request.delete(`${MEM0_URL}/memories/${memoryId}`);
      await expectOk(res);
    }
  });

  test("DELETE /memories → cleanup test user memories", async ({ request }) => {
    const res = await request.delete(`${MEM0_URL}/memories?user_id=${TEST_USER_ID}`);
    await expectOk(res);
