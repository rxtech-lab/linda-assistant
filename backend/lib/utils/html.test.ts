...
mock.module("@/lib/s3", () => ({
  uploadBufferToS3: async (buffer: Buffer, _contentType: string, filename: string) => {
...
describe("stripBase64DataUris", () => {
  ...
  test("strips multiple base64 images", () => {
    ...
    expect(stripBase64DataUris(html)).toBe('<img src=""><p>text</p><img src="">');
  });
  ...
  test("handles mixed normal and base64 src attributes", () => {
    const html = '<img src="https://example.com/ok.png"><img src="data:image/png;base64,abc">';
    expect(stripBase64DataUris(html)).toBe('<img src="https://example.com/ok.png"><img src="">');
  });
...
describe("replaceInlineImages", () => {
  test("replaces cid: references with S3 URLs from cidMap", async () => {
    ...
    const cidMap = new Map([["<inline-image-1.png>", "https://s3.example.com/attachment1.png"]]);
    const result = await replaceInlineImages(html, cidMap);
    expect(result.html).toBe('<img src="https://s3.example.com/attachment1.png">');
    expect(result.inlineImages).toHaveLength(0);
  });

  test("replaces multiple cid: references", async () => {
    const html = '<img src="cid:img1.png"><p>text</p><img src="cid:img2.jpg">';
  ...
  test("uploads base64 data URIs to S3 and returns them as inline images", async () => {
    ...
    expect(result.html).toBe('<img src="https://s3.example.com/email-inline-images/inline.png">');
  ...
  test("returns multiple inline images from base64 extraction", async () => {
    const html = '<img src="data:image/png;base64,abc"><img src="data:image/jpeg;base64,def">';
    const result = await replaceInlineImages(html, new Map());
  ...
  test("handles both cid and base64 in same HTML", async () => {
    const html = '<img src="cid:att1.png"><img src="data:image/jpeg;base64,/9j/4A==">';
    const cidMap = new Map([["<att1.png>", "https://s3.example.com/att1.png"]]);
    const result = await replaceInlineImages(html, cidMap);
    expect(result.html).toContain('<img src="https://s3.example.com/att1.png">');
