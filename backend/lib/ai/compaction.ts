...
export function extractTextFromMessage(msg: ModelMessage): string {
  ...
  return parts
    .map((p) => {
      if (p.type === "text" && typeof p.text === "string") return p.text as string;
      if (p.type === "tool-call")
 ...
 */
export function findSafeSplitPoint(messages: ModelMessage[], targetIndex: number): number {
 ...
 */
export function truncateMessage(msg: ModelMessage, maxTokens: number): ModelMessage {
 ...
 */
export async function compactOversizedMessage(text: string, maxTokens: number): Promise<string> {
...
export async function prepareMessages(opts: {
  ...
  const { sessionId, userId, messages, systemPrompt, assigneeId, onEvent } = opts;
  ...
  try {
    const { oldMessages, recentMessages, splitIndex } = splitMessages(messages);
    ...
    mem0
      ...
      .catch((err) => console.warn("[compaction] mem0 addMemories failed:", err));
    ...
    const result = await handleOversizedLastMessage(compactedMessages, contextWindow, onEvent);
...
async function handleOversizedLastMessage(
  ...
  try {
    ...
    const summarized = await compactOversizedMessage(originalText, Math.floor(maxSingleTokens));
