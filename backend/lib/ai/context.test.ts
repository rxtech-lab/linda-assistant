...
describe("estimateTokenCount", () => {
  ...
  test("scales with message count", () => {
    ...
    const tenMsgs = Array.from({ length: 10 }, (_, i) => textMsg("user", `Message ${i}`));
    expect(estimateTokenCount(tenMsgs)).toBeGreaterThan(estimateTokenCount(oneMsg));
  });

  test("scales with message length", () => {
    const short = [textMsg("user", "Hi")];
    const long = [textMsg("user", "A".repeat(10000))];
    expect(estimateTokenCount(long)).toBeGreaterThan(estimateTokenCount(short) * 10);
  });
