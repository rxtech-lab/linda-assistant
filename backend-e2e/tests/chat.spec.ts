	...
	.serial("Confirmation flow", () => {
		...
		test("test 1: get current time + approve confirmation + continue chat", async ({
			...
			try {
				...
				const textDeltas = stream.events.filter(
				...
				const timeResultParts = findToolResultParts(
					messages,
					"get_current_time",
				);
	...
	.serial("Stream behavior", () => {
		...
		test("test 8: no stream listener — poll DB until assistant replies, then connect and receive nothing", async ({
			...
			try {
				...
				console.log(
					"Test 8 passed: no stream events after generation completed",
				);
			} finally {
				stream.cancel();
			}
	...
	.serial("Question tool", () => {
		...
		test("test 9: ask question + answer + follow up", async ({
			...
			try {
				...
				const toolResults = stream.events.filter(
					(e) =>
						e.event === "tool-result" && e.data.toolName === "ask_question",
				);
				expect(toolResults.length).toBeGreaterThanOrEqual(1);
				...
				await sendMessage(
					assigneeId,
					"What did I answer? Do not use any tools.",
				);
				await stream.waitForDone();
				...
				const questionResultParts = findToolResultParts(
					messages,
					"ask_question",
				);
		...
		test("test 11: ask question + user skips + keeps chatting = auto reject", async ({
			...
			const stream = consumeStream(assigneeId, {
				...
				onMessage: async (evt) => {
					if (evt.event === "question_required") {
						console.log(
							`Question required but ignoring: ${evt.data.questionId}`,
						);
					}
				},
			});
...
test.describe
	.serial("Location tool", () => {
		test.setTimeout(300_000);

		test("test 12: get location + share + follow up", async ({
			assigneeId,
		}) => {
			const stream = consumeStream(assigneeId, {
				timeout: 180_000,
			...
			try {
				...
				const toolResults = stream.events.filter(
					(e) =>
						e.event === "tool-result" && e.data.toolName === "get_location",
				);
				expect(toolResults.length).toBeGreaterThanOrEqual(1);
				...
				const locationResultParts = findToolResultParts(
					messages,
					"get_location",
				);
		...
		test("test 13: get location + reject + follow up", async ({
			assigneeId,
		}) => {
			const stream = consumeStream(assigneeId, {
				timeout: 180_000,
		...
		test("test 14: get location + user skips + keeps chatting = auto reject", async ({
			...
			const stream = consumeStream(assigneeId, {
				...
				onMessage: async (evt) => {
					if (evt.event === "location_required") {
						console.log(
							`Location required but ignoring: ${evt.data.toolCallId}`,
						);
					}
				},
			});
